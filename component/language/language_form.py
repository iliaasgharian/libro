...
class LanguageForm(QWidget):
    language_saved = pyqtSignal()
    def __init__(self, right_stack=None):
        ...
        self.setup_ui()
    def save(self):
        s=LanguageDataAdapter.update(id=int(self.txt_id.text()),
                                   name=self.txt_name.text())
        print("Language {} saved".format(self.txt_id.text()))
        self.language_saved.emit()
    def setup_ui(self):
    ...
    def on_language_clicked(self, name):
        if self.right_stack is not None:
            self.right_stack.setCurrentWidget(self)

        language=LanguageDataAdapter.search(name=name)[0]


        self.txt_id.setText(str(language.id))
        self.txt_name.setText(language.name)

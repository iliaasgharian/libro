...
class DesignerForm(QWidget):
    designer_saved = pyqtSignal()
    def __init__(self, right_stack=None):
        ...
        self.setup_ui()
    def save(self):
        s=DesignerDataAdapter.update(id=int(self.txt_id.text()),
                                   name=self.txt_name.text(),
                                   nationality=self.txt_nationality.text(),
                                   birthdate = datetime.strptime(self.txt_birth_date.text(), "%Y-%m-%d").date())
        print("Desginer {} saved".format(self.txt_id.text()))
        self.designer_saved.emit()
    def setup_ui(self):
        ...
        til=QHBoxLayout()
        til.setSpacing(10)
        label_til = QLabel("CoverDesigners")
        label_til.setObjectName("title")
        label_til.setFixedWidth(110)
    ...
    def on_designers_clicked(self, name):
        if self.right_stack is not None:
            self.right_stack.setCurrentWidget(self)
        designer = DesignerDataAdapter.search(name=name)[0]

        self.txt_id.setText(str(designer.id))
        self.txt_name.setText(designer.name)
        self.txt_nationality.setText(designer.nationality)
        self.txt_birth_date.setText(designer.birthdate)

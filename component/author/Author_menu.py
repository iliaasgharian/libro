...
class AuthorWin(QWidget):
    ...
    def on_search(self, text):
        if text.strip():
            authors = AuthorDataAdapter.search(name=text)
        else:
    ...
    def load_authors(self, authors):
        ...
        for author in authors:
            btn = QPushButton(author.name)
            btn.setObjectName("boButton")

            btn.clicked.connect(
                lambda checked, name=author.name: self.author_clicked.emit(name)
            )
    ...
    def refresh(self):
        ...
        self.load_authors(AuthorDataAdapter.get_all())

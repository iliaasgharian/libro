...
class ResourceForm(QWidget):
    ...
    def __init__(self, right_stack=None):
        ...
        self.setup_ui()
    def save(self):
        s=ResourcesDataAdapter.update(id=int(self.txt_id.text()),
                                   name=self.txt_name.text(),)
        print("Resource {} saved".format(self.txt_id.text()))
        self.resource_saved.emit()
    def setup_ui(self):
    ...
    def on_resource_clicked(self, name):
        if self.right_stack is not None:
            self.right_stack.setCurrentWidget(self)

        resource=ResourcesDataAdapter.search(name=name)[0]


        self.txt_id.setText(str(resource.id))
        self.txt_name.setText(resource.name)
